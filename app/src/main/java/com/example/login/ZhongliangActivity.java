...
public class ZhongliangActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_zhongliang);

    initView();
    btAdd.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View view) {
        Intent intent=new Intent(ZhongliangActivity.this,AddZhongliangActivity.class);
    ...
    btExchange.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        if (TextUtils.isEmpty(etExchangeQuantity.getText().toString())){
          Toast.makeText(ZhongliangActivity.this,"请添加换算数量",Toast.LENGTH_SHORT).show();
          return;
        }
        ...
        if (!zhongliangDao.isExist(sp1.getSelectedItem().toString(),sp2.getSelectedItem().toString())) {
          Toast.makeText(ZhongliangActivity.this,"请添加换算关系",Toast.LENGTH_SHORT).show();
        }else {
          ArrayList<ExchangeBean> value = zhongliangDao.findValue(sp1.getSelectedItem().toString(), sp2.getSelectedItem().toString());
